...
export default function DashboardPage() {
  ...
  // Driver phone state
  const [driverPhoneCode, setDriverPhoneCode] = useState('+251')
  ...
  const handleAddNewDriver = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userData = {
        ...
        phoneNumber: (formData.get('phoneNumberFull') as string) || (formData.get('phoneNumber') as string) || undefined,
        role: 'Driver',
      }
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <button 
                onClick={() => { setShowAssignDriverForm(true); setShowAddDriverSection(true) }}
          ...
          <div className="bg-white rounded-xl p-6 border border-gray-200">
            ...
            {/* Chart — real fuel data by day of week */}
            {(() => {
              // Group fuel records by day of week (last 7 days)
              const now = new Date()
              const days = Array.from({ length: 7 }, (_, i) => {
                const d = new Date(now)
                d.setDate(now.getDate() - 6 + i)
                return { label: d.toLocaleString('en-US', { weekday: 'short' }).toUpperCase(), date: d, liters: 0 }
              })
              // allFuelRecords is not in scope here — use fuelUsage from stats as total
              // Show a flat bar if no per-day data available
              const maxVal = Math.max(...days.map(d => d.liters), 1)
              return (
                <div className="h-64 flex items-end justify-between gap-2 md:gap-3 border-b border-gray-200 pb-2">
                  {days.map((bar, index) => (
                    <div key={index} className="flex-1 flex flex-col items-center gap-2 group">
                      <div className="relative w-full flex flex-col items-center">
                        <div className="w-full bg-[#1B3D2F] rounded-t transition-all hover:bg-[#152e22]"
                          style={{ height: `${Math.max((bar.liters / maxVal) * 200, bar.liters > 0 ? 8 : 4)}px` }}>
                        </div>
                      </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onFocus={() => {
                        if (makeInput.length > 0 && makeSuggestions.length > 0) setShowMakeSuggestions(true)
                      }}
                    />
                    {showMakeSuggestions && (
                      <ul className="absolute z-20 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-48 overflow-y-auto">
                        {makeSuggestions.map(s => (
                          <li key={s}
                            onMouseDown={() => { setMakeInput(s); setShowMakeSuggestions(false) }}
                            className="px-4 py-2 text-sm hover:bg-[#1B3D2F]/10 cursor-pointer flex items-center gap-2"
                          >
                            <svg className="w-3.5 h-3.5 text-[#1B3D2F]" fill="currentColor" viewBox="0 0 20 20"><path d="M8 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM15 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z"/><path d="M3 4a1 1 0 00-1 1v10a1 1 0 001 1h1.05a2.5 2.5 0 014.9 0H10a1 1 0 001-1V5a1 1 0 00-1-1H3zM14 7a1 1 0 00-1 1v6.05A2.5 2.5 0 0115.95 16H17a1 1 0 001-1v-5a1 1 0 00-.293-.707l-2-2A1 1 0 0015 7h-1z"/></svg>
                            {s}
                          </li>
                        ))}
                      </ul>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      onFocus={() => {
                        if (modelInput.length > 0 && modelSuggestions.length > 0) setShowModelSuggestions(true)
                      }}
                    />
                    {showModelSuggestions && (
                      <ul className="absolute z-20 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-48 overflow-y-auto">
                        {modelSuggestions.map(s => (
                          <li key={s}
                            onMouseDown={() => { setModelInput(s); setShowModelSuggestions(false) }}
                            className="px-4 py-2 text-sm hover:bg-[#1B3D2F]/10 cursor-pointer flex items-center gap-2"
                          >
                            <svg className="w-3.5 h-3.5 text-[#1B3D2F]" fill="currentColor" viewBox="0 0 20 20"><path d="M8 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM15 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0z"/><path d="M3 4a1 1 0 00-1 1v10a1 1 0 001 1h1.05a2.5 2.5 0 014.9 0H10a1 1 0 001-1V5a1 1 0 00-1-1H3zM14 7a1 1 0 00-1 1v6.05A2.5 2.5 0 0115.95 16H17a1 1 0 001-1v-5a1 1 0 00-.293-.707l-2-2A1 1 0 0015 7h-1z"/></svg>
                            {s}
      ...
      {showAssignDriverForm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            {/* Form Content */}
            <div className="p-6">
              {/* Toggle between Add Driver and Import CSV */}
                        ...
                        <div>
                          ...
                          <div className="flex gap-2">
                            <select
                              ...
                              onChange={e => { setDriverPhoneCode(e.target.value); validateDriverPhone(e.target.value, driverPhoneNumber) }}
                              className="w-36 px-2 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1B3D2F] focus:border-transparent outline-none text-sm bg-white"
                            >
                              {COUNTRY_CODES.map(c => (
                            ...
                            <div className="flex-1 relative">
                              <input
                                ...
                                onChange={e => {
                                  ...
                                  setDriverPhoneNumber(val)
                                  validateDriverPhone(driverPhoneCode, val)
                                }}
                                onBlur={() => validateDriverPhone(driverPhoneCode, driverPhoneNumber)}
                              />
                              {/* hidden input that submits the full number */}
                              <input type="hidden" name="phoneNumberFull" value={driverPhoneNumber ? `${driverPhoneCode}${driverPhoneNumber.replace(/\D/g, '')}` : ''} />
                              {driverPhoneNumber && !driverPhoneError && (
                          ...
                          {driverPhoneError && <p className="mt-1 text-xs text-red-600 flex items-center gap-1"><svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clipRule="evenodd"/></svg>{driverPhoneError}</p>}
                          {driverPhoneNumber && !driverPhoneError && (
                            <p className="mt-1 text-xs text-gray-400">Full number: {driverPhoneCode}{driverPhoneNumber.replace(/\D/g, '')}</p>
                          )}
                        </div>
