...
export default function ApprovalsPage() {
  ...
  const getStatusFromState = (state: string) => {
    // Pending: trips waiting for department approval
    if (state === 'PENDING_DEPARTMENT') return 'pending'
    // Approved: trips that department has approved (moved to next stage or beyond)
    if (state === 'PENDING_COLLEGE' || state === 'PENDING_DEAN' || state === 'PENDING_PRESIDENT' || 
        state === 'APPROVED_FOR_ALLOCATION' || state === 'CAR_ALLOCATED' || state === 'PENDING_TRANSPORT_CONFIRM' ||
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#1B3D2F]">Trip Requests</h1>
          ...
          <div key={request.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <div className="px-4 md:px-6 pb-4 md:pb-6">
              {approvedIds.has(request.id) ? (
                <div className="w-full px-4 py-2 md:py-2.5 bg-green-50 border border-green-200 rounded-lg flex items-center justify-center gap-2">
                  <svg className="w-4 h-4 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                  </svg>
                  <span className="text-green-600 text-xs md:text-sm font-semibold">Approved</span>
                </div>
              ) : (
                <button
          ...
          <div className="flex min-h-full items-center justify-center p-3 md:p-4">
            <div className="relative bg-white rounded-xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-4 md:p-6 space-y-4 md:space-y-6">
                ...
                {/* Actions */}
                {getStatusFromState(selectedRequest.state) === 'pending' && !approvedIds.has(selectedRequest.id) && (
                ...
                {approvedIds.has(selectedRequest.id) && (
                  <div className="flex items-center justify-center gap-3 pt-3 md:pt-4 border-t border-gray-200">
                    <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center animate-bounce">
                      <svg className="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                      </svg>
                    </div>
                    <span className="text-green-600 font-semibold text-sm md:text-base">Approved Successfully!</span>
                  </div>
                )}
              </div>
            </div>
