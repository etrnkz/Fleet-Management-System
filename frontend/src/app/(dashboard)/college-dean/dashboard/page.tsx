...
export default function DashboardPage() {
  ...
  const fleetStatus = vehicles.slice(0, 5).map((vehicle: any) => {
    ...
    return {
      ...
      statusColor: isActive ? 'text-[#1B3D2F]' : isInUse ? 'text-blue-600' : 'text-orange-600',
      percentage: pct,
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-[#1B3D2F]">Welcome, {user?.name?.split(' ')[0] || 'Dean'}</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
            {[
              { label: 'Total Trips (College)', value: trips.length, color: 'text-[#1B3D2F]', dot: 'bg-[#1B3D2F]', desc: 'All trips in your college' },
          ...
          <div className="grid lg:grid-cols-3 gap-4 md:gap-6">
            <div className="bg-white rounded-xl border border-gray-200 p-4 md:p-6">
              ...
              <div className="space-y-3">
                {[
                  { label: 'Completed', count: trips.filter(t => t.state === 'COMPLETED').length, color: 'bg-[#1B3D2F]' },
                    ...
                    <div className="flex justify-between items-end h-32 md:h-40 gap-2 md:gap-3">
                      {monthlyChart.map(({ month, count, pct }) => (
                        <div key={month} className="flex flex-col justify-end items-center gap-1 flex-1">
                          ...
                          <div className="w-full bg-[#152e22] rounded-t shadow-sm transition-all" style={{ height: pct, minHeight: count > 0 ? '4px' : '2px' }}></div>
                ...
                <div key={fleet.id} className="border-b border-gray-100 pb-3 md:pb-4 last:border-0 last:pb-0">
                  ...
                  <div className="flex items-center gap-2 md:gap-3">
                    <div className="flex-1 bg-gray-200 rounded-full h-1.5 md:h-2">
                      <div 
                        className={`h-1.5 md:h-2 rounded-full ${fleet.status === 'Active' || fleet.status === 'ACTIVE' ? 'bg-[#152e22]' : fleet.status === 'IN_USE' ? 'bg-blue-500' : 'bg-orange-500'}`}
