...
export default function ApprovalsPage() {
  ...
  const handleApprove = async (request: any) => {
    try {
      ...
      setApprovedIds(prev => new Set(prev).add(request.id))
              ...
              {selectedTab === 'pending' && (
                <div className="flex gap-2 mt-3 pt-3 border-t border-gray-100" onClick={e => e.stopPropagation()}>
                  {approvedIds.has(request.id) ? (
                    <div className="flex-1 py-2 bg-green-50 border border-green-200 rounded-lg flex items-center justify-center gap-2">
              ...
              <div className="bg-gray-50 border-t border-gray-200 p-3 md:p-4 flex flex-col sm:flex-row justify-end gap-2 md:gap-3">
                {approvedIds.has(selectedRequest.id) ? (
                  <div className="flex items-center justify-center gap-3 w-full py-2">
                    ...
                    <span className="text-green-600 font-semibold text-base">Approved Successfully!</span>
                  </div>
                ) : (
                  <>
                    <button
