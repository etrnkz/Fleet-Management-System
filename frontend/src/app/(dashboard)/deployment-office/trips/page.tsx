...
export default function TripsPage() {
  ...
  const handleSelectVehicle = (vehicleId: string) => {
    ...
    setAssignmentData(prev => ({ ...prev, vehicleId, driverId }))
  }
  ...
  const handleSaveAssignment = async () => {
    ...
    try {
      await tripApi.assignVehicleAndDriver(
        ...
        assignmentData.estimatedFuelCost ? Number(assignmentData.estimatedFuelCost) : 0,
        assignmentData.estimatedDistance ? Number(assignmentData.estimatedDistance) : 0
      )
  ...
  const getFilteredTrips = () => {
    return tripsList.filter(trip => {
      const matchesSearch = searchQuery === '' || 
        ...
        (trip.requestedBy || trip.requester?.name || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
        (trip.department || trip.requester?.department?.name || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
        (trip.destination || '').toLowerCase().includes(searchQuery.toLowerCase())

      const tripStatus = (trip.status || trip.state || '').toLowerCase().replace(/_/g, '-').replace(' ', '-')
      // Map backend states to filter values
      const normalizedStatus = 
        tripStatus === 'approved-for-allocation' ? 'pending-assignment' :
        tripStatus === 'car-allocated' || tripStatus === 'assigned' ? 'assigned' :
        tripStatus
          ...
          <div key={trip.id} className="bg-white rounded-xl border border-gray-200 p-4 md:p-6 hover:shadow-lg transition-shadow">
            <div className="flex flex-col lg:flex-row lg:items-center gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{trip.requestedBy || trip.requester?.name || 'N/A'} • {trip.department || trip.requester?.department?.name || 'N/A'}</p>
                  </div>
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${trip.statusColor || 'bg-yellow-100 text-yellow-700'}`}>
                    {trip.status || (trip.state === 'APPROVED_FOR_ALLOCATION' ? 'Pending Assignment' : (trip.state || '').replace(/_/g, ' '))}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-gray-700"><span className="font-medium">Date:</span> {trip.startDate || (trip.startDateTime ? new Date(trip.startDateTime).toLocaleDateString() : 'N/A')} to {trip.endDate || (trip.endDateTime ? new Date(trip.endDateTime).toLocaleDateString() : 'N/A')}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span className="text-gray-700"><span className="font-medium">Duration:</span> {trip.duration || (trip.startDateTime && trip.endDateTime ? `${Math.ceil((new Date(trip.endDateTime).getTime() - new Date(trip.startDateTime).getTime()) / (1000*60*60*24))} days` : 'N/A')}</span>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                    </svg>
                    <span className="text-gray-700"><span className="font-medium">Passengers:</span> {trip.passengers || trip.passengerCount || 'N/A'}</span>
                  </div>
                </div>
              ...
              <div className="flex lg:flex-col gap-2">
                ...
                {(trip.status === 'Pending Assignment' || trip.state === 'APPROVED_FOR_ALLOCATION') && (
                  <button
      ...
      {showAssignModal && selectedTrip && (
        <div className="fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-6">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="font-medium text-gray-900">{selectedTrip.requestNumber || selectedTrip.id}</p>
                </div>
                <div>
                  <span className="text-gray-600">Requester:</span>
                  <p className="font-medium text-gray-900">{selectedTrip.requestedBy || selectedTrip.requester?.name || 'N/A'}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900">{selectedTrip.passengers || selectedTrip.passengerCount || 'N/A'} people</p>
                </div>
                <div>
                  <span className="text-gray-600">Duration:</span>
                  <p className="font-medium text-gray-900">{selectedTrip.duration || (selectedTrip.startDateTime && selectedTrip.endDateTime ? `${Math.ceil((new Date(selectedTrip.endDateTime).getTime() - new Date(selectedTrip.startDateTime).getTime()) / (1000*60*60*24))} days` : 'N/A')}</p>
                </div>
                <div>
                  <span className="text-gray-600">Start Date:</span>
                  <p className="font-medium text-gray-900">{selectedTrip.startDate || (selectedTrip.startDateTime ? new Date(selectedTrip.startDateTime).toLocaleDateString() : 'N/A')}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-6">
              <label className="block text-sm font-semibold text-gray-800 mb-1">Select Vehicle</label>
              <p className="text-xs text-[#1B3D2F] bg-[#1B3D2F]/8 px-3 py-2 rounded-lg mb-3 flex items-center gap-2">
                <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                Select a vehicle — the driver assigned to it by Transport Admin will be automatically allocated.
              </p>
              ...
              return driver ? (
                <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-sm font-bold text-green-800">Auto-assigned driver</span>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white rounded-lg p-3 border border-green-200">
                      <p className="text-xs text-gray-500 mb-1">Vehicle</p>
                      <p className="text-sm font-semibold text-gray-900">{vehicle.plateNumber}</p>
                      <p className="text-xs text-gray-500">{vehicle.make} {vehicle.model}</p>
                    </div>
                    <div className="bg-white rounded-lg p-3 border border-green-200">
                      <p className="text-xs text-gray-500 mb-1">Driver (auto)</p>
                      <p className="text-sm font-semibold text-gray-900">{driver.user?.name || driver.name}</p>
                      <p className="text-xs text-gray-500">{driver.user?.phoneNumber || driver.licenseNumber || ''}</p>
                    </div>
                  </div>
                </div>
              ) : null
            ...
            <div className="mb-6 grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={e => setAssignmentData(prev => ({ ...prev, estimatedFuelCost: e.target.value }))}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={e => setAssignmentData(prev => ({ ...prev, estimatedDistance: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-[#1B3D2F] focus:border-transparent outline-none"
                />
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full p-6 max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${selectedTrip.statusColor || 'bg-yellow-100 text-yellow-700'}`}>
                  {selectedTrip.status || (selectedTrip.state === 'APPROVED_FOR_ALLOCATION' ? 'Pending Assignment' : (selectedTrip.state || '').replace(/_/g, ' '))}
                </span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900">{selectedTrip.department || selectedTrip.requester?.department?.name || selectedTrip.requester?.college?.name || 'N/A'}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium text-gray-900">{selectedTrip.startDate || (selectedTrip.startDateTime ? new Date(selectedTrip.startDateTime).toLocaleString() : 'N/A')}</p>
                </div>
                <div>
                  <span className="text-sm text-gray-600">End Date</span>
                  <p className="font-medium text-gray-900">{selectedTrip.endDate || (selectedTrip.endDateTime ? new Date(selectedTrip.endDateTime).toLocaleString() : 'N/A')}</p>
                </div>
                <div>
                ...
                <div className="md:col-span-2">
                  ...
                  <p className="font-medium text-gray-900">{selectedTrip.purpose || 'N/A'}</p>
                </div>
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              <div className="space-y-3">
                {(selectedTrip.approvals || []).length > 0 ? (selectedTrip.approvals || []).map((approval: any, index: number) => (
                  ...
                  <div className="p-3 bg-[#1B3D2F]/10 rounded-lg">
                    <p className="text-sm text-gray-600">State: <span className="font-medium text-gray-900">{(selectedTrip.state || '').replace(/_/g, ' ')}</span></p>
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Assignment Details */}
            {(selectedTrip.vehicle || selectedTrip.allocatedVehicle) && (selectedTrip.driver || selectedTrip.allocatedDriver) && (
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="bg-[#1B3D2F]/10 rounded-lg p-4">
                    ...
                    {(() => { const v = selectedTrip.vehicle || selectedTrip.allocatedVehicle; return (<><p className="font-medium text-gray-900">{v?.name || `${v?.make || ''} ${v?.model || ''}`.trim() || 'N/A'}</p><p className="text-sm text-gray-600">{v?.plate || v?.plateNumber || 'N/A'}</p></>)})()}
                  </div>
                  <div className="bg-blue-50 rounded-lg p-4">
                    ...
                    {(() => { const d = selectedTrip.driver || selectedTrip.allocatedDriver; return (<><p className="font-medium text-gray-900">{d?.name || d?.user?.name || 'N/A'}</p><p className="text-sm text-gray-600">{d?.phone || d?.user?.phoneNumber || 'N/A'}</p></>)})()}
                  </div>
                </div>
              </div>
            )}
