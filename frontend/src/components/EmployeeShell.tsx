...
const navItems: { href: string; label: string; requestSection?: boolean; icon: (active: boolean) => ReactNode }[] = [
  ...
  {
    href: '/employee/dashboard?section=request',
    label: 'New Request',
    requestSection: true,
    icon: (active) => (
      <svg
...
function EmployeeShellInner({ children, title, subtitle, headerActions }: EmployeeShellProps) {
  ...
  const isActive = (href: string, label: string, requestSection?: boolean) => {
    ...
    if (href.startsWith('/employee/dashboard')) return false
        ...
        <header className="sticky top-0 z-30 w-full h-16 flex flex-wrap gap-y-2 justify-between items-center px-4 sm:px-8 bg-[var(--fa-surface)]/95 backdrop-blur-md border-b border-[var(--fa-outline-variant)]/20 shadow-sm transition-colors duration-300">
          ...
          <div className="flex items-center gap-4 sm:gap-6 ml-auto">
            ...
            <button
              type="button"
              onClick={() => toggleTheme()}
              className="p-2 rounded-lg transition-colors hover:bg-[var(--fa-surface-container)] text-xl leading-none"
              aria-label="Toggle theme"
              style={{ color: '#1B3D2F', minWidth: '36px', minHeight: '36px' }}
            >
              {isDark ? (
              ...
              <div className="relative">
                ...
                {profileDropdownOpen && (
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-52 bg-[var(--fa-surface)] border border-[var(--fa-outline-variant)]/20 rounded-xl shadow-xl z-40 overflow-hidden">
                      <div className="px-4 py-3 border-b border-[var(--fa-outline-variant)]/20">
                        <p className="text-sm font-semibold text-[var(--fa-on-surface)] truncate">{user?.name}</p>
                        <p className="text-xs text-[var(--fa-secondary)] truncate">{user?.email}</p>
                      </div>
                      <div className="p-1">
                        <Link
                          href="/employee/profile"
                          onClick={() => setProfileDropdownOpen(false)}
                          className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-[var(--fa-on-surface)] hover:bg-[var(--fa-surface-container)] transition-colors"
                        >
                          <svg className="w-4 h-4 text-[var(--fa-secondary)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                          </svg>
                          My Profile
                        </Link>
                      </div>
                      <div className="p-1 border-t border-[var(--fa-outline-variant)]/20">
                        <button
                          type="button"
