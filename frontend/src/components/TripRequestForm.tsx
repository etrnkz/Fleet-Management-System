...
export default function TripRequestForm({ onSuccess, onCancel, showToast }: TripRequestFormProps) {
  ...
  // Phone state
  const [primaryCode, setPrimaryCode] = useState('+251')
  const [primaryNumber, setPrimaryNumber] = useState('')
  const [primaryError, setPrimaryError] = useState('')
  const [altCode, setAltCode] = useState('+251')
  const [altNumber, setAltNumber] = useState('')
  const [altError, setAltError] = useState('')
  const [emergencyPhoneCode, setEmergencyPhoneCode] = useState('+251')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const pErr = validatePhone(primaryCode, primaryNumber)
    if (pErr) { setPrimaryError(pErr); showToast('Please fix the phone number errors', 'error'); return }
    if (!primaryNumber.trim()) { showToast('Primary contact phone is required', 'error'); return }

    const fullPrimary = `${primaryCode}${primaryNumber.replace(/\D/g, '')}`
    const fullAlt = altNumber ? `${altCode}${altNumber.replace(/\D/g, '')}` : undefined
  ...
  return (
    <div className="space-y-8 max-w-4xl mx-auto">
      ...
      {showSuccess ? (
        <div className="bg-white rounded-2xl border border-green-200 shadow-lg p-16 flex flex-col items-center justify-center min-h-[400px]">
          <div className="w-28 h-28 rounded-full bg-green-100 flex items-center justify-center mb-6">
            <svg className="w-16 h-16 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
            </svg>
          </div>
          <h3 className="text-2xl font-bold text-green-600 mb-3">Request Submitted!</h3>
          <p className="text-gray-600 text-center max-w-sm leading-relaxed">
            Your trip request has been submitted successfully and is now pending approval from your department head.
          </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <PhoneInput
              ...
              number={altNumber}
              error={altError}
              onCodeChange={c => { setAltCode(c); setAltError(validatePhone(c, altNumber)) }}
              onNumberChange={n => { setAltNumber(n); setAltError(validatePhone(altCode, n)) }}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <PhoneInput
                ...
                onCodeChange={c => { setEmergencyPhoneCode(c); setEmergencyPhoneError(validatePhone(c, emergencyPhoneNumber)) }}
                onNumberChange={n => { setEmergencyPhoneNumber(n); setEmergencyPhoneError(validatePhone(emergencyPhoneCode, n)) }}
              />
            </div>
          </div>
