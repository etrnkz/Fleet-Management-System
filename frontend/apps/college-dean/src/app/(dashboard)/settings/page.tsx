...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      department: parsedData.department || currentUser?.department?.name || '',
      college: parsedData.college || currentUser?.college?.name || '',
    })
    setProfileImage(parsedData.profileImage || currentUser?.profileImage || null)
    setLoading(false)
  }, [])
          ...
          <div className="flex gap-1 px-6 min-w-max">
            {tabs.map(({ id, label }) => (
              <button key={id} onClick={() => setActiveTab(id)}
                className={`py-4 px-3 border-b-2 font-medium text-sm transition-colors whitespace-nowrap ${activeTab === id ? 'border-[#1B3D2F] text-[#1B3D2F]' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
                {label}
              </button>
            ))}
          </div>
                ...
                <div className="relative">
                  <div className="w-24 h-24 rounded-full overflow-hidden bg-gray-200 border-4 border-[#1B3D2F]">
                    {profileImage ? <img src={profileImage} alt="Profile" className="w-full h-full object-cover" /> : (
                      <div className="w-full h-full flex items-center justify-center bg-[#152e22] text-white text-2xl font-bold">{(formData.name || 'U').charAt(0).toUpperCase()}</div>
                    )}
                  </div>
            ...
            <div className="space-y-6">
              ...
              {inviteMode === 'email' ? (
                <div>
                  ...
                  <textarea value={inviteEmails} onChange={(e) => setInviteEmails(e.target.value)} rows={5} placeholder="john@university.edu, jane@university.edu"
              ...
              <div>
                ...
                <textarea value={inviteMessage} onChange={(e) => setInviteMessage(e.target.value)} rows={3} placeholder="Welcome to the Fleet Management System!"
