...
export default function MaintenanceNotificationsPage() {
  ...
  const typeIcon = (type: string) => {
    ...
    if (type?.toLowerCase().includes('complet'))
      return { icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z', color: 'text-green-500', bg: 'bg-green-100' }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        {unreadCount > 0 && (
          <button
            type="button"
            onClick={markAllRead}
            className="px-4 py-2 text-sm text-blue-600 border border-blue-600/30 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 font-medium"
          >
            Mark all read
          </button>
        )}
            ...
            return (
              <div
                ...
                className={`bg-white dark:bg-gray-800 rounded-xl border p-4 cursor-pointer transition-all hover:shadow-md ${
                  !n.isRead ? 'border-blue-200 dark:border-blue-600 bg-blue-50/30 dark:bg-blue-900/10' : 'border-gray-200 dark:border-gray-700'
                }`}
              >
