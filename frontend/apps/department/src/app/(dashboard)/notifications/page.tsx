...
export default function NotificationsPage() {
  ...
  const typeIcon = (type: string) => {
    ...
    if (type?.toLowerCase().includes('approv')) return { color: 'text-[#1B3D2F]', bg: 'bg-[#1B3D2F]/15', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        {([['all', 'All'], ['unread', `Unread (${unreadCount})`], ['read', 'Read']] as const).map(([val, label]) => (
          <button key={val} onClick={() => setFilter(val)}
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${filter === val ? 'bg-[#1B3D2F] text-white' : 'bg-white border border-gray-300 text-gray-600 hover:bg-gray-50'}`}>
            {label}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="bg-white rounded-xl border border-gray-200 p-4 animate-pulse h-20" />)}</div>
      ) : filtered.length === 0 ? (
          ...
          {filtered.map(n => {
            ...
            return (
              <div key={n.id} onClick={() => !n.isRead && markRead(n.id)}
                className={`bg-white rounded-xl border p-4 transition-all hover:shadow-md ${!n.isRead ? 'border-[#1B3D2F]/20 bg-[#1B3D2F]/10/30 cursor-pointer' : 'border-gray-200'}`}>
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {!n.isRead && <span className="w-2 h-2 bg-[#152e22] rounded-full flex-shrink-0 mt-1.5"></span>}
                    </div>
