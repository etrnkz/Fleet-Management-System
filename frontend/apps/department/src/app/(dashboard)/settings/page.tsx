            ...
            {['profile', 'account', 'invite'].map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={`py-4 px-2 border-b-2 font-medium text-sm transition-colors capitalize ${
                  activeTab === tab ? 'border-[#1B3D2F] text-[#1B3D2F]' : 'border-transparent text-gray-500 hover:text-gray-700'
                }`}>
          ...
          {activeTab === 'invite' && (
            <div className="space-y-6">
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setInviteMode('csv')}
                  ...
                  {inviteResult.invited.length > 0 && (
                    <div className="bg-[#1B3D2F]/10 border border-[#1B3D2F]/20 rounded-lg p-4">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {inviteResult.invited.map((e) => <span key={e} className="text-xs bg-[#1B3D2F]/15 text-[#1B3D2F] px-2 py-1 rounded">{e}</span>)}
                      </div>
                    </div>
                  )}
