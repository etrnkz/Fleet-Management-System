...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const savedEmail = localStorage.getItem('employee_rememberedEmail')
      if (savedEmail) {
        setEmail(savedEmail)
        setRememberMe(true)
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/auth/login`, {
        ...
        body: JSON.stringify({ email, password, appType: 'employee' }),
      })
      ...
      if (rememberMe) {
        localStorage.setItem('employee_rememberedEmail', email)
      } else {
        localStorage.removeItem('employee_rememberedEmail')
      }
    ...
    <div className="min-h-screen flex">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <h1 className="text-lg font-bold text-emerald-700 tracking-tight">Employee Portal</h1>
              </div>
            </div>
            <h2 className="text-2xl font-bold text-emerald-700 font-serif tracking-tight">Secure sign in</h2>
            <p className="text-gray-600 text-sm mt-2 font-medium">Use your assigned employee credentials to access the portal.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <input
                ...
                placeholder="employee@haramaya.edu.et"
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-emerald-700 flex-col justify-center px-12 text-white overflow-hidden">
        ...
        <div className="relative z-10 max-w-md">
          <p className="text-emerald-100 text-xs font-semibold uppercase tracking-[0.2em] mb-4">Employee access</p>
          ...
          <p className="mt-6 text-xs text-white/60 uppercase tracking-widest font-semibold">Authorized employees only</p>
        </div>
      </div>
    </div>
  )
