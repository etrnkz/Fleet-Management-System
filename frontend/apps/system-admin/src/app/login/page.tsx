...
export default function LoginPage() {
  ...
  useEffect(() => {
    const remembered = localStorage.getItem('systemAdminRememberedUser')
    if (remembered) {
      const data = JSON.parse(remembered)
      if (new Date(data.expiry) > new Date()) {
        setSavedCredentials({ email: data.email, password: data.password })
      } else {
        localStorage.removeItem('systemAdminRememberedUser')
      }
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.access_token) throw new Error(response.message || 'Login failed')
      ...
      if (rememberMe) {
        ...
        localStorage.setItem('systemAdminRememberedUser', JSON.stringify({ email, password, expiry: expiry.toISOString() }))
      } else {
        localStorage.removeItem('systemAdminRememberedUser')
      }
    ...
    <div className="min-h-screen flex">
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <h1 className="text-lg font-bold text-[#1B3D2F] tracking-tight">System Admin Portal</h1>
              </div>
            </div>
            <h2 className="text-2xl font-bold text-[#1B3D2F] tracking-tight">Secure sign in</h2>
            <p className="text-gray-600 text-sm mt-2">Use your assigned system administrator credentials.</p>
          </div>
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-[#152e22] flex-col justify-center px-12 text-white overflow-hidden">
        ...
        <div className="relative z-10 max-w-md">
          <p className="text-[#D1E1FF] text-xs font-semibold uppercase tracking-[0.2em] mb-4">System Admin access</p>
          ...
          <p className="mt-4 text-white/85 text-sm leading-relaxed font-medium">
            Full system administration — manage users, configure roles, monitor system health, and oversee all fleet operations across the university.
          </p>
