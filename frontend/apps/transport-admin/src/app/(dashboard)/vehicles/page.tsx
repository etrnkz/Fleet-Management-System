  ...
  return (
    ...
    <div className="p-2 sm:p-4 lg:p-6 h-full flex flex-col gap-3 sm:gap-4 lg:gap-6">
      ...
      <div className="hidden lg:flex flex-1 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex-col">
        <div className="overflow-x-auto flex-1">
          <table className="w-full min-w-max">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="px-3 xl:px-6 py-3 text-left text-[10px] xl:text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Vehicle ID</th>
                <th className="px-3 xl:px-6 py-3 text-left text-[10px] xl:text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Plate Number</th>
                <th className="px-3 xl:px-6 py-3 text-left text-[10px] xl:text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Make & Model</th>
                <th className="px-3 xl:px-6 py-3 text-left text-[10px] xl:text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Type</th>
                <th className="px-3 xl:px-6 py-3 text-left text-[10px] xl:text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Status</th>
                <th className="px-3 xl:px-6 py-3 text-left text-[10px] xl:text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Driver</th>
                <th className="px-3 xl:px-6 py-3 text-left text-[10px] xl:text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Fuel Type</th>
                <th className="px-3 xl:px-6 py-3 text-left text-[10px] xl:text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Mileage</th>
                <th className="px-3 xl:px-6 py-3 text-left text-[10px] xl:text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">VIP zone</th>
                <th className="px-3 xl:px-6 py-3 text-left text-[10px] xl:text-xs font-semibold text-gray-600 uppercase tracking-wider whitespace-nowrap">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {filteredVehicles.length === 0 ? (
                ...
                <tr key={vehicle.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-3 xl:px-6 py-3 xl:py-4 whitespace-nowrap">
                    <span className="text-xs xl:text-sm font-semibold text-gray-900">{vehicle.vehicleId || vehicle.plateNumber}</span>
                  </td>
                  <td className="px-3 xl:px-6 py-3 xl:py-4 whitespace-nowrap">
                    <span className="text-xs xl:text-sm text-gray-900 font-medium">{vehicle.plateNumber}</span>
                  </td>
                  <td className="px-3 xl:px-6 py-3 xl:py-4 whitespace-nowrap">
                    <div className="flex flex-col">
                      <span className="text-xs xl:text-sm text-gray-900">{vehicle.make}</span>
                      <span className="text-[10px] xl:text-xs text-gray-500">{vehicle.model} ({vehicle.year})</span>
                    </div>
                  </td>
                  <td className="px-3 xl:px-6 py-3 xl:py-4 whitespace-nowrap">
                    <span className="text-xs xl:text-sm text-gray-600">{vehicle.vehicleType || 'N/A'}</span>
                  </td>
                  <td className="px-3 xl:px-6 py-3 xl:py-4 whitespace-nowrap">
                    {vehicle.onTrip ? (
                      <span className="px-2 xl:px-3 py-0.5 xl:py-1 rounded-full text-[10px] xl:text-xs font-medium bg-blue-100 text-blue-700">
                        On Duty
                      </span>
                    ) : (
                      <span className={`px-2 xl:px-3 py-0.5 xl:py-1 rounded-full text-[10px] xl:text-xs font-medium ${getStatusColor(vehicle.status)}`}>
                        {vehicle.status}
                      </span>
                    )}
                  </td>
                  ...
                  <td className="px-3 xl:px-6 py-3 xl:py-4 whitespace-nowrap">
                    <span className="text-xs xl:text-sm text-gray-600">{vehicle.fuelType}</span>
                  </td>
                  <td className="px-3 xl:px-6 py-3 xl:py-4 whitespace-nowrap">
                    <span className="text-xs xl:text-sm text-gray-600">{vehicle.currentMileage ? `${vehicle.currentMileage.toLocaleString()} km` : 'N/A'}</span>
                  </td>
                  <td className="px-3 xl:px-6 py-3 xl:py-4 whitespace-nowrap">
                    {vehicle.vipGeoRestrictionEnabled ? (
                      <span className="px-2 py-0.5 xl:py-1 rounded text-[10px] xl:text-xs font-medium bg-amber-100 text-amber-800">
                        On ({Array.isArray(vehicle.restrictedZones) ? vehicle.restrictedZones.length : 0})
                      </span>
                    ) : (
                  ...
                  <td className="px-3 xl:px-6 py-3 xl:py-4 whitespace-nowrap">
                    <button
                      type="button"
                      onClick={() => openGeofenceEditor(vehicle)}
                      className="text-xs xl:text-sm font-medium text-[#1B3D2F] hover:text-[#152e22] whitespace-nowrap"
                    >
                      VIP / zones
                    </button>
                  </td>
        ...
        <div className="px-3 xl:px-6 py-3 xl:py-4 border-t border-gray-200 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs xl:text-sm text-gray-600">Showing {filteredVehicles.length} of {vehicles.length} vehicles</p>
          <button onClick={loadVehicles} className="flex items-center gap-2 px-3 xl:px-4 py-2 bg-[#1B3D2F] text-white rounded-lg hover:bg-[#152e22] transition-colors text-xs xl:text-sm font-medium">
