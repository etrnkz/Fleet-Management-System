...
export class TrackingController {
  ...
  async bulkUpdateLocations(
    ...
    return { count: savedLocations.length, message: `${savedLocations.length} locations saved successfully` };
  }
  ...
  getActiveViewers(@Param('tripId', ParseUUIDPipe) tripId: string) {
    return { tripId, activeViewers: this.trackingGateway.getActiveViewers(tripId) };
  }
}
