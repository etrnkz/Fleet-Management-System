...
## 11. MVC / Layered Architecture

NestJS follows a strict layered architecture. Each module has:
...
┌──────────────────────▼──────────────────────────────┐
│              Controller Layer (C)                    │
│  - Route handling, HTTP method mapping               │
│  - Input validation via DTOs + ValidationPipe        │
│  - Auth guards (JwtAuthGuard, RolesGuard)            │
│  - No business logic                                 │
└──────────────────────┬──────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────┐
│              Service Layer (M - Business)            │
...
│                                                      │
│  Key services:                                       │
│  ├── TripsService (trip lifecycle)                   │
│  ├── TripApprovalService (approval workflow)         │
│  ├── TripAllocationService (vehicle/driver assign)   │
│  ├── VehiclesService                                 │
│  ├── DriversService                                  │
│  ├── MaintenanceService                              │
│  ├── FuelService                                     │
...
│  ├── AuditService                                    │
│  └── WorkflowService (timeout/scheduling)           │
└──────────────────────┬──────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────┐
│              Repository Layer (M - Data)             │
...
```
trips/
├── trips.controller.ts      ← Routes, guards, DTOs
├── gate-scan.controller.ts  ← Gate-specific routes
├── trips.service.ts         ← Core trip lifecycle
├── trip-approval.service.ts ← Approval workflow (SRP)
├── trip-allocation.service.ts ← Allocation logic (SRP)
├── trips.module.ts          ← DI wiring
├── dto/                     ← Input validation schemas
├── entities/                ← DB schema definitions
└── utils/                   ← Pure functions (QR parsing)
```

### Single Responsibility Principle
Each service has one reason to change:
- `TripsService` — trip CRUD and lifecycle state transitions
- `TripApprovalService` — approval validation and transitions
- `TripAllocationService` — vehicle/driver availability checks
- `NotificationsService` — notification creation and delivery
- `AuditService` — audit log recording
- `WorkflowService` — timeout scheduling via Bull queues

### Data Transfer Objects (DTOs)
All input is validated via class-validator DTOs:
- `CreateTripDto` — trip creation with field validation
- `AllocateTripDto` — allocation with UUID validation
- `CreateUserDto` — user creation with password strength + phone format
- All DTOs use `whitelist: true` — unknown fields are rejected
